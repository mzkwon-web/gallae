(function(global){
  ...
  function classifyDoksanGuroRow(row){
    if(!row) return {direction_validated:false,toward_guro:false,realtime_usable:false,reason:'missing_row'};
    ...
    const realtimeUsable=row.realtime_usable===true;
    return {direction_validated:directionValidated,realtime_usable:realtimeUsable,freshness:row.freshness||'unknown',age_seconds:row.age_seconds==null?null:Number(row.age_seconds),toward_guro:directionValidated&&realtimeUsable,checks,subwayId:row.subwayId||null,updnLine:row.updnLine||null,destination:row.bstatnNm||null,trainLineNm:row.trainLineNm||null,btrainNo:row.btrainNo||null,service_type:row.btrainSttus||null,barvlDt:Number.isFinite(barvl)?barvl:null,realtime_eta_seconds:realtimeUsable&&Number.isFinite(barvl)&&barvl>0?barvl:null,station_distance:stationDistance,arrival_code:arrivalCode||null,arrival_state:arrivalStateFromCode(arrivalCode),can_board_current_snapshot:realtimeUsable&&directionValidated&&arrivalCode==='1',already_departed_current_snapshot:arrivalCode==='2',arrival_priority:arrivalPriority(arrivalCode,stationDistance),arrival_message:row.arvlMsg2||null,current_location:row.arvlMsg3||null,subway_heading:row.subwayHeading==null?null:row.subwayHeading,observed_at:row.observed_at||row.recptnDt||null,ordkey:row.ordkey||null,evidence:validEvidence([{source:'api',text:'독산역 1호선 실시간 원응답의 방향·행선·현재 위치·도착상태 관측값'},{source:'rule',text:'구로 방향은 실제 probe에서 확인된 상행 + 가산디지털단지방면 + FID/TID 조합으로만 판별'},{source:'rule',text:'arvlCd는 서울시 공식 도착코드 정의에 따라 진입/도착/출발/전역상태를 구분'},...(!realtimeUsable?[{source:'constraint',text:'freshness 기준을 통과하지 못한 snapshot은 LIVE·ETA·추천 근거에서 제외'}]:[]),...(arrivalCode==='2'?[{source:'constraint',text:'현재 역 출발(arvlCd=2) 열차는 현재 탑승 후보에서 제외'}]:[]),...(Number.isFinite(barvl)&&barvl<=0?[{source:'constraint',text:'관측된 barvlDt=0은 v0.1에서 실시간 도착초로 사용하지 않음'}]:[]),...(row.subwayHeading==null?[{source:'constraint',text:'이번 실제 probe에서 subwayHeading은 null이므로 방향 판별 근거로 사용하지 않음'}]:[])])};
  }
