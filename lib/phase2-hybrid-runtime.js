(function(global){
  ...
  function buildRuntimeCandidate(options){
    ...
    // The API can return a row whose recptnDt is older than the newly fetched snapshot.
    // A calculated boarding time that is already before the snapshot cannot represent
    // a train that is still boardable "now" and must not seed the rolling ETA.
